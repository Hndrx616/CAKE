...
if (!class_exists('cake_dbColumn')):
	require_once(CAKE_BASE_CLASS_DIR.'column.php');
endif;
...
class cake_entity {
	...
	function setGuid($string) {
		...
		return cake_lib::setStringGuid($string);
	...
	function delete($value = '', $col = 'id') {	
		...
		if ( $status ){
			if ($this->isCachable()) {
				cake_coreAPI::debug('about to remove from cache');
				$cache = cake_coreAPI::cacheSingleton();
	...
	function getByColumn($col, $value) {
		...
		if (!empty($cache_obj)) {
			...
			$db->selectColumn('*');
			cake_coreAPI::debug("Col: $col, value: $value");	
			$db->where($col, $value);
			...
			if (!empty($properties)) {
				...
				// add to cache			
				$this->addToCache($col);
				cake_coreAPI::debug('entity loaded from db');		
			}
		} 
	}
	...
	function setTableName($name, $namespace = 'cake_') {
	...
	function setCachable($seconds = '') {
		...
		if (!$seconds) {
			...
			//$seconds = cake_coreAPI::getSetting('base', 'default_cache_expiration_period');
			$seconds = 604800;
		}
	...
	function isCachable() {
		...
		if (cake_coreAPI::getSetting('base', 'cache_objects')) {
	...
	function createTable() {
		...
		if ($status == true):
			cake_coreAPI::notice(sprintf("%s Table Created.", $this->getTableName()));
			return true;
		else:
			cake_coreAPI::notice(sprintf("%s Table Creation Failed.", $this->getTableName()));
			return false;
		endif;
	...
	function generateRandomUid($seed = '') {
		...
		return cake_lib::generateRandomUid();
