...
class cake_auth extends cake_base {
	...
	function __construct() {
		...
		cake_coreAPI::registerStateStore('u', time()+3600*24*365*10, '', '', 'cookie');
		cake_coreAPI::registerStateStore('p', time()+3600*2, '', '', 'cookie');
		
		parent::__construct();
		$this->eq = cake_coreAPI::getEventDispatch();	
	}
	...
	function authenticateUser() {
		...
		if (cake_coreAPI::getCurrentUser()->isAuthenticated()) {
			$ret = true;
		} elseif (cake_coreAPI::getRequestParam('apiKey')) {			
			// auth user by api key
			$ret = $this->authByApiKey(cake_coreAPI::getRequestParam('apiKey'));
		} elseif (cake_coreAPI::getRequestParam('pk') && cake_coreAPI::getStateParam('u')) {
			// auth user by temporary passkey. used in forgot password situations
			$ret = $this->authenticateUserByUrlPasskey(cake_coreAPI::getRequestParam('pk'));
		} elseif (cake_coreAPI::getRequestParam('user_id') && cake_coreAPI::getRequestParam('password')) {
			// auth user by login form input
			$ret = $this->authByInput(cake_coreAPI::getRequestParam('user_id'), cake_coreAPI::getRequestParam('password'));
		} elseif (cake_coreAPI::getStateParam('u') && cake_coreAPI::getStateParam('p')) {
			// auth user by cookies
			$ret = $this->authByCookies(cake_coreAPI::getStateParam('u'), cake_coreAPI::getStateParam('p'));
			// bump expiration time
			//cake_coreAPI::setState('p', '', cake_coreAPI::getStateParam('p'));
		} else {
			$ret = false;
			cake_coreAPI::debug("Could not find any credentials to authenticate with.");
		}
	...
	function authByApiKey( $key ) {
		...
		$key = cake_sanitize::cleanMd5( $key );
		
		if ( $key ) {
		
			// fetch user object from the db
			$this->u = cake_coreAPI::entityFactory('base.user');
	...
	function authByCookies($user_id, $password) {
		...
		// set credentials
		$this->credentials['user_id'] = cake_sanitize::cleanUserId( $user_id );
	...
	function authByInput($user_id, $password) {
		...
		//cake_coreAPI::debug(print_r($this->credentials, true));
		$ret = $this->isUser();
	...
	function authenticateUserByUrlPasskey($user_id, $passkey) {
		...
		$passkey = cake_sanitize::cleanMd5( $passkey );
	...
	function deleteCredentials() {
		...
		return cake_coreAPI::clearState('p');
	}
	...
	function encryptPassword($password) {
		...
		return cake_lib::encryptPassword($password);
	...
	function reconstructAuthCredential( $user_id ) {
		...
		$u = cake_coreAPI::entityFactory('base.user');
	...
	function generateAuthCredential($user_id, $password, $expiration = '', $scheme = 'auth') {
		...
		$key = cake_coreAPI::saltedHash( $user_id . $frag . $expiration, $scheme );
		...
		$hash = cake_lib::hash( $algo, $user_id . $expiration, $key );
