...
abstract class cake_module extends cake_base {
	 ...
	 * Used by controllers to see if an update error was becuase it needs to be applied from the command line instead of via the browser.
	 * @var boolean 
	 */
	...
	function registerEventHandler($event_name, $handler_name, $method = 'notify', $dir = 'handlers') {
		...
		if (!is_object($handler_name)) {
			...
			//$handler = &cake_lib::factory($handler_dir,'cake_', $handler_name);
			$handler_name = cake_coreAPI::moduleGenericFactory($this->name, $dir, $handler_name, $class_suffix = null, $params = '', $class_ns = 'cake_');	
		}
	...
	function registerFilter($filter_name, $handler_name, $method = '', $priority = 10, $dir = 'filters') {
		...
		if ( is_object( $handler_name ) ) {
			...
			cake_coreAPI::registerFilter($filter_name, array($handler_name, $method), $priority);
		
		// if it's a static method name
		} elseif ( strpos( $handler_name, '::') ) {
			
			cake_coreAPI::registerFilter($filter_name, $handler_name, $priority);
		...
		// else try to create the class object
			...
			if ( ! class_exists( $handler_name ) ) {		
				...
				//$handler = &cake_lib::factory($handler_dir,'cake_', $handler_name);
				$class = cake_coreAPI::moduleGenericFactory($this->name, $dir, $handler_name, $class_suffix = null, $params = '', $class_ns = 'cake_');
			}
			
			// register
			cake_coreAPI::registerFilter($filter_name, array($class, $method), $priority);
		}
...
if ( ! is_object( $handler_name ) ) {
		...
		return cake_coreAPI::registerFilter($filter_name, array($handler_name, $method), $priority);
*/
	}
	...
	function install() {
		...
		if (!empty($this->entities)):
			...
			foreach ($this->entities as $k => $v) {
				...
				$entity = cake_coreAPI::entityFactory($this->name.'.'.$v);
	...
	function update() {
		...
		$files = cake_lib::listDir(CAKE_DIR.'modules'.'/'.$this->name.'/'.'updates', false);
		//print_r($files);
		...
		foreach ($files as $k => $v) {
			...
			if ($seq > $current_schema_version):
				...
				if ($seq <= $this->required_schema_version):
					$this->updates[$seq] = cake_coreAPI::updateFactory($this->name, substr($v['name'], 0, -4));
					// if the cli update mode is required and we are not running via cli then return an error.
					cake_coreAPI::debug('cli update mode required: '.$this->updates[$seq]->isCliModeRequired());
					if ($this->updates[$seq]->isCliModeRequired() === true && !defined('CAKE_CLI')) {
						//set flag in module
						$this->update_from_cli_required = true;
						cake_coreAPI::notice("Aborting update $seq. This update must be applied using the command line interface.");
						return false;
					}
	...
	function isSchemaCurrent() {
		...
		cake_coreAPI::debug("$this->name Schema version is $current_schema");
		cake_coreAPI::debug("$this->name Required Schema version is $required_schema");
	...
	function getSchemaVersion() {
		...
		$current_schema = cake_coreAPI::getSetting($this->name, 'schema_version');
	...
	/**Adds an event processor class to the processor array. This is used to determin which class to use to process a particular event*/
	 ...
	 * @param 	$foreign_key_name the name of the foreign key column that should be used to relate the metric entity to the dimension's entity. 
	 ...
	 * These are tracking properties that are derived from the Server environment and should be added to all tracking tracking events as they are recieved.
	 *
	 ...
	 *
	 * 		'REMOTE_HOST'		=> array(
	 *			'default_value'		=> array( 'cake_trackingEventHelpers::remoteHostDefault' ),
	 *			'required'			=> true,
	 *			'data_type'			=> 'string',
	 *			'filter'			=> true
	 *		)
	 ...
	 * This method is called by a module's constructor and should be redefined in a concrete module class.
	 */
	...
	/**
	 * Registers a new package of files to be built by the 'build' CLI command.
