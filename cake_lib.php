...
class cake_lib {
	...
	function errorHandler($msg) {
		require_once(CAKE_PEARLOG_DIR . '/Log.php');
		...
		switch ($params['period']) {
			...
			case "day":
				return sprintf("%s, %d%s %s",
							cake_lib::get_month_label($params['month']),
							$params['day'],
							cake_lib::setDaySuffix($params['day']),
							$params['year']				
						);
				break;
			...
			case "date_range":
				return sprintf("%s, %d%s %s - %s, %d%s %s",
							...
							cake_lib::setDaySuffix($params['day']),
							$params['year'],
							cake_lib::get_month_label($params['month2']),
							$params['day2'],
							cake_lib::setDaySuffix($params['day2']),
							$params['year2']					
						);
				break;
		}
	...
	public static function get_period_label($period) {
		...
		$periods = cake_lib::reporting_periods();
	...
	public static function inputFilter($input, $options = array() ) {
		...
		return cake_sanitize::cleanInput( $input, $options );		
	}
	...
	public static function factory($class_dir, $class_prefix, $class_name, $constructorArguments = array(), $class_suffix = '') {
        ...
        /*Attempt to include a version of the named class, but don't treat a failure as fatal.*/
    ...
    public static function singleton($class_dir, $class_prefix, $class_name, $conf = array()) {
        ...
        if (!isset($instance)) {
        	...
        	$instance = cake_lib::factory($class_dir, $class_prefix, $class_name, $conf);
        }
	...
	public static function moduleRequireOnce($module, $file) {
		...
		return require_once(CAKE_BASE_DIR.'/modules/'.$module.'/'.$file.'.php');
	}
	...
	public static function moduleFactory($modulefile, $class_suffix = null, $params = '') {
		...
		$class = 'cake_'.$file.$class_suffix;
		
		// Require class file if class does not already exist
		if(!class_exists($class)):	
			cake_lib::moduleRequireOnce($module, $file);
		endif;
	...
	public static function redirectToView($data) {
		...
		cake_lib::redirectBrowser($new_url);
	}
	...
	public static function displayView($data, $params = array()) {
		...
		$view =  cake_lib::moduleFactory($data['view'], 'View', $params);
	...
	public static function setStringGuid($string) {
		...
		if ( $string ) {
			...
			if ( cake_coreAPI::getSetting('base', 'use_64bit_hash') && PHP_INT_MAX == '9223372036854775807') {
	...
	public static function listDir($start_dir='.', $recursive = true) {
		...
		if (is_dir($start_dir)):
			...
			while (($file = readdir($fh)) !== false) {
				...
				if (is_dir($filepath)):
					if ($recursive === true):
						$files = array_merge($files, cake_lib::listDir($filepath));
					endif;
				else:
	...
	public static function makeDateArray($result, $format) {
		...
		if (!empty($result)) {
			...
			return cake_lib::makeDates($timestamps, $format);
	...
	public static function encryptPassword($password) {
		...
		//return cake_coreAPI::saltedHash( $password, 'auth');
	}
	...
	public static function utf8Encode($string) {
		...
		if ( cake_lib::checkForUtf8( $string ) ) {
			return $string; 
		} else { 
    ...
    /**Use this function to parse out the url and query array element from a url.*/
	...
	public static function generateRandomUid($seed='') {
		...
		$time = (string) time();
		$random = cake_lib::zeroFill( mt_rand( 0, 999999 ), 6 );
		if ( defined('CAKE_SERVER_ID') ) {
			$server = cake_lib::zeroFill( CAKE_SERVER_ID, 3 );
		} else {
