...
class cake_db extends cake_base {
	...
	function _insertQuery() {
		...
		cake_coreAPI::profile($this, __FUNCTION__, __LINE__);
		$this->_setSql(sprintf(CAKE_SQL_INSERT_ROW, $this->_sqlParams['table'], $sql_cols, $sql_values));
		cake_coreAPI::profile($this, __FUNCTION__, __LINE__);
		$ret = $this->_query();	
		cake_coreAPI::profile($this, __FUNCTION__, __LINE__);
		return $ret;
	...
	function _updateQuery() {
		...
		$this->_setSql(sprintf(CAKE_SQL_UPDATE_ROW, $this->_sqlParams['table'], $set, $this->_makeWhereClause()));
	...
	function _deleteQuery() {
		...
		$this->_setSql(sprintf(CAKE_SQL_DELETE_ROW, $this->_sqlParams['table'], $this->_makeWhereClause()));
		...
		if ( ! empty( $params ) ) {
			...
			foreach ($params as $k => $v) {			
				...
				switch (strtolower($v['operator'])) {
					...
					case '=~':
						$constraint .= sprintf("%s %s '%s'",$v['name'], CAKE_SQL_REGEXP, $this->prepare( $v['value'] ) );
						break;
						
					case '!~':
						$constraint .= sprintf("%s %s '%s'",$v['name'], CAKE_SQL_NOTREGEXP, $this->prepare( $v['value'] ) );
						break;
	...
	function _formatResults($results) {
		...
		switch ($this->_sqlParams['result_format']) {
				...
				case "single_row":
					...
					break;	
				case "inverted_array":
					return cake_lib::deconstruct_assoc($results);
					break;
				default:
					return $results;
					break;
	...
	function dropTable($table_name) {
		...
		return $this->query(sprintf(CAKE_SQL_DROP_TABLE, $table_name));
	...
	function alterTableType($table_name, $engine) {
		...
		return $this->query(sprintf(CAKE_SQL_ALTER_TABLE_TYPE, $table_name, $engine));
	...
	function renameTable($table_name, $new_table_name) {
		...
		return $this->query(sprintf(CAKE_SQL_RENAME_TABLE, $table_name, $new_table_name));
	}
	
	/** Renames column idempotent */
	function renameColumn($table_name, $old, $new, $defs) {
	
		return $this->query(sprintf(CAKE_SQL_RENAME_COLUMN, $table_name, $old, $new, $defs));
	}

	/**Adds new column to table idempotent*/
	function addColumn($table_name, $column_name, $column_definition) {
	
		return $this->query(sprintf(CAKE_SQL_ADD_COLUMN, $table_name, $column_name, $column_definition));
	}
	
	/**Drops a column from a table*/
	function dropColumn($table_name, $column_name) {
	
		return $this->query(sprintf(CAKE_SQL_DROP_COLUMN, $table_name, $column_name));
	...
	function modifyColumn($table_name, $column_name, $column_definition) {
		...
		return $this->query(sprintf(CAKE_SQL_MODIFY_COLUMN, $table_name, $column_name, $column_definition));
	}
	
	/**Adds index to a column*/
	function addIndex($table_name, $column_name, $index_definition = '') {
	
		return $this->query(sprintf(CAKE_SQL_ADD_INDEX, $table_name, $column_name, $index_definition));
	}
	
	/** Adds index to a column*/
	function dropIndex($table_name, $column_name) {
	
		return $this->query(sprintf(CAKE_SQL_DROP_INDEX, $column_name, $table_name));
	}
	...
	function createTable($entity) {
		...
		switch ($options['table_type']) {
			...
			case "disk":
				$table_type = CAKE_DTD_TABLE_TYPE_DISK;
				break;
			case "memory":
				$table_type = CAKE_DTD_TABLE_TYPE_MEMORY;
				break;
			default:
				$table_type = CAKE_DTD_TABLE_TYPE_DEFAULT;
	
		}
		
		$table_options .= sprintf(CAKE_DTD_TABLE_TYPE, $table_type);
		...
		if (!array_key_exists('character_encoding', $options)) {
			...
			$options['character_encoding'] = CAKE_DTD_CHARACTER_ENCODING_UTF8;			
		}
	
		$table_options .= sprintf(' ' . CAKE_DTD_TABLE_CHARACTER_ENCODING, $options['character_encoding']);
	
		return $this->query(sprintf(CAKE_SQL_CREATE_TABLE, $entity->getTableName(), $columns, $table_options));
	}
	
	/**Begins a SQL transaction statement*/
	function beginTransaction() {
	
		return $this->query(CAKE_SQL_BEGIN_TRANSACTION);
	}
	
	/**Ends a SQL transaction statement*/
	function endTransaction() {
	
		return $this->query(CAKE_SQL_END_TRANSACTION);
	}
	
	function count($column_name) {
		
		return sprintf(CAKE_SQL_COUNT, $column_name);
	}
	
	function sum($column_name) {
		
		return sprintf(CAKE_SQL_SUM, $column_name);
	}
	
	function distinct($column_name) {
		
		return sprintf(CAKE_SQL_DISTINCT, $column_name);
	}
	
	function division($numerator, $denominator) {
		
		return sprintf(CAKE_SQL_DIVISION, $numerator, $denominator);
	}
	
	function round($value) {
	
		return sprintf(CAKE_SQL_ROUND, $value);
	}
	
	function average($value) {
	
		return sprintf(CAKE_SQL_AVERAGE, $value);
	}
