<?php
//*******************************************************************
//Template Name: cake_template.php
//Author: Stephen Hilliard (c) 2016
//Developer: Stephen Hilliard (c) 2016
//license http://www.gnu.org/licenses/gpl-3.0.html GPL v3 or later
// @project: cake_template
//*******************************************************************/

if (!class_exists('Template')) {
	require_once(CAKE_INCLUDE_DIR.'template_class.php');
}

if (!class_exists('cake_lib')) {
	require_once(CAKE_BASE_DIR.'/cake_lib.php');
}

if (!class_exists('cake_sanitize')) {
	require_once(CAKE_BASE_CLASS_DIR.'sanitize.php');
}

class cake_template extends Template {
	...
	function __construct( $module = null, $caller_params = array() ) {
		...
		$this->time_now = cake_lib::time_now();
	}
	
	function _setTemplateDir($module) {
	
		// set module template dir
		$this->module_template_dir = CAKE_DIR.'modules'.'/' . $module . '/'.'templates'.'/';
		...
		$this->theme_template_dir = CAKE_THEMES_DIR.$this->config['theme'].'/';
	...
	function getTemplatePath($module, $file) {
		...
		if ($file == null) {
			cake_coreAPI::error('No template file was specified.');
			return false;
		} else {
	...
	function truncate ($str, $length=10, $trailing='...')  {
      ...
      return cake_lib::truncate ($str, $length, $trailing); 
	}
	
	function get_month_label($month) {
		
		return cake_lib::get_month_label($month);
	}
	...
	function choose_browser_icon($browser_type, $height= '32', $width='') {
		...
		if ( file_exists( CAKE_MODULES_DIR . $file ) ) {
	...
	function getBrowserIcon($browser_family, $size = '128x128', $module = 'base') {
		...
		if (file_exists(CAKE_MODULES_DIR.$module.'/i/browsers/'.$size.'/'.$browser_family.'.png')) {
	...
	/**
	 * Makes navigation links by checking whether or not the view that is rendering the template is not the view being refered to in the link.
	...
	function makeWikiLink($page) {
		...
		return sprintf($this->config['cake_wiki_link_template'], $page);
	}
	...
	function makeParamString($params = array(), $add_state = false, $format = 'query', $namespace = true) {
		...
		switch($format) {
			...
			case 'cookie':
				...
				$string = cake_lib::implode_assoc('=>', '|||', $all_params);
				break;
	...
	function makeLink($params = array(), $add_state = false, $url = '', $xml = false, $add_nonce = false) {
		...
		if ($add_nonce) {
			...
			$all_params['nonce'] = cake_coreAPI::createNonce($action);
		}
		...
		if (!empty($all_params)):
			...
			foreach ($all_params as $n => $v) {
				...
				$get .= $this->config['ns'].cake_sanitize::escapeForDisplay($n).'='.cake_sanitize::escapeForDisplay($v);
	...
	function escapeForXml($string) {
		...
		$string = cake_lib::escapeNonAsciiChars($string);
		return $string;
	}
	...
	function makeImageLink($path, $absolute = false) {
		...
		if ($absolute === true) {
			$url = cake_coreAPI::getSetting('base', 'modules_url');
		} else {
			$url = cake_coreAPI::getSetting('base', 'modules_url');
		}
	...
	function getWidget($do, $params = array(), $wrapper = true, $add_state = true) {
		...
		return cake_coreAPI::performAction($do, $final_params);
	}
	
	function getInpageWidget($do, $params = array()) {
	
		return cake_template::getWidget($do, $params, 'inpage');
	...
	function getSparkline($metric, $metric_col, $period = '', $height = 25, $width = 250, $map = array(), $add_state = true) {
		...
		$map['height'] = $height;
		$map['width'] = $width;
		return cake_template::getWidget('base.widgetSparkline', $map, false, $add_state);
	
	}
		
	function makeJson($array) {
		
		$reserved_words = cake_coreAPI::getSetting('base', 'reserved_words');
	...
	function makePagination($pagination, $map = array(), $add_state = true, $template = '') {
		...
		if ($pagination['max_page_num'] > 1) {
			...
			$pages = '<div class="cake_pagination"><UL>';
			...
			for ($i = 1; $i <= $pagination['max_page_num'];$i++) {
				...
				if ($pagination['page_num'] != $i) {
					...
					$new_map = $map;
					$new_map['page'] = $i;
					$link = sprintf('<LI class="cake_reportPaginationControl"><a href="%s">%s</a></LI>', 
														$this->makeLink($new_map, $add_state), 
														$i);
				
				} else {
					
					$link = sprintf('<LI class="cake_reportPaginationControl">%s</LI>', $i);
				}
	...
	function displayMetricInfobox($params = array()) {
		...
		$count = cake_coreAPI::executeApiCommand($params);
		$params['period'] = 'last_thirty_days';
		$params['dimensions'] = 'date';
		$trend = cake_coreAPI::executeApiCommand($params);
	...
	function createNonceFormField($action) {
		...
		return sprintf(
				...
				cake_coreAPI::getSetting('base', 'ns'), 
				cake_coreAPI::createNonce($action));
	}
	...
	function out($output, $sanitize = true, $decode_special_entities = false) {
		...
		if ( $sanitize ) {
			$output = cake_sanitize::escapeForDisplay($output);
	...
	function formatCurrency($value) {
		return cake_lib::formatCurrency( $value, cake_coreAPI::getSetting( 'base', 'currencyLocal' ) );
	}
	
	function getCurrentUser() {
		
		return cake_coreAPI::getCurrentUser();
	}
	
	function getLatestActions( $startDate, $endDate, $siteId, $visitorId = '', $sessionId = '', $width = '300px', $resultsPerPage = 25, $page = 1 ) {
		
		$la = cake_coreAPI::executeApiCommand(array(
