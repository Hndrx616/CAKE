<?php
//*******************************************************************
//Template Name: cake_httpRequest.php
//Author: Stephen Hilliard (c) 2016
//Developer: Stephen Hilliard (c) 2016
//license http://www.gnu.org/licenses/gpl-3.0.html GPL v3 or later
// @project: cake_httpRequest
//*******************************************************************/

if(!class_exists('Snoopy')) {
	require_once(CAKE_INCLUDE_DIR.'/Snoopy.class.php');
}

require_once(CAKE_HTTPCLIENT_DIR.'http.php');
...
class cake_http {
	...
	function __construct() {
		...
		$this->crawler->agent = cake_coreAPI::getSetting('base', 'cake_user_agent');
		//$this->crawler->agent = "Firefox";
		//cake_coreAPI::debug('hello from cake_http constructor');
		return;
	
	}
	
	function fetch($uri) {
		//cake_coreAPI::debug('hello from cake_http fetch');
	...
	function testFetch($url) {
		...
		$http= new http_class;
		cake_coreAPI::debug('hello cake_http testfetch method');
		/* Connection timeout */
		$http->timeout=0;
		...
		$http->debug=1;
		$http->user_agent = cake_coreAPI::getSetting('base', 'cake_user_agent');
		...
		//		{
					...
					cake_coreAPI::debug(HtmlSpecialChars($body));
		//		}
	...
	function extract_anchor($link) {
		...
		if (isset($matches[0])) {
			$this->anchor_info =  array('anchor_tag' => $matches[0], 'anchor_text' => cake_lib::inputFilter($matches[0]));
	...
	function extract_anchor_snippet($link){
		...
		if(!empty($this->anchor_info['anchor_tag'])) {
			...
			$first_part = strip_tags(cake_lib::inputFilter($first_part));
			...
			//$part1 = cake_lib::inputFilter($part1);
			...
			$part2 = substr(strip_tags(cake_lib::inputFilter($part2)),0, $this->snip_len);

			// Put humpty dumpy back together again and create actual snippet
			$snippet =  $this->snip_str.$part1.' <span class="snippet_anchor">'.cake_lib::inputFilter($this->anchor_info['anchor_tag']).'</span> '.$part2.$this->snip_str;
   ...
   function SetupHTTP()
	{
		if(!IsSet($this->http))
		{
			...
			$this->http->user_agent =  cake_coreAPI::getSetting('base', 'cake_user_agent');
			$this->http->timeout = 3;
	...
	function GetRequestResponse(&$response)
	{
		...
		$this->http->Close();
		cake_coreAPI::debug('http response code: '.$this->http->response_status);
		return($response);
	}
